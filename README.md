...
Optionally **seal** the finished bundle so a third party can later confirm it is
byte-for-byte what was graded:

```bash
python3 scripts/verify.py <output_dir> --seal        # writes provenance_manifest.json
python3 scripts/verify.py <output_dir> --check-seal  # re-hash -> PASS / ALTERED
```

This is **integrity, not authenticity**: hashing proves a bundle is unchanged since it
was sealed, not *who* sealed it — an unsigned manifest can be regenerated by anyone with
write access. Treat a PASS as a convenience check that supports, not replaces, your own
chain-of-custody controls.

A finished brief is a snapshot: true (as far as `verify.py` could establish) on the day it
was written. Later, **re-check** whether that snapshot has gone stale — a cited paper
retracted, a preprint superseded, a source no longer resolving:

```bash
python3 scripts/verify.py <output_dir> --recheck --write   # writes refresh_diff.json + refresh_report.md
python3 scripts/verify.py <output_dir> --recheck --offline # honest "we couldn't check" (cache only, no network)
```

`--recheck` re-resolves every source's publication identity through the **same** shipped
metadata adapters, re-runs the **same** `verify()`, and emits a deterministic diff: which
sources changed status, which claims are consequently weakened, whether the quality-gate
tier moved. The report then gains a **"What changed since `<date>`"** panel. This is a
**manual, point-in-time re-check — not monitoring**: it schedules nothing and watches
nothing on its own. Its honesty rule is that *"checked, unchanged"*, *"checked, changed"*,
and *"could not be checked"* are three distinct states that are never conflated — a source
that could not be reached is reported `not_rechecked`, never `unchanged`. `--recheck --write`
re-seals the bundle so `--check-seal` still passes afterward.
...
| `decision_tripwires.json` | 1/final (optional) | Flat `T-###` revisit checklist bound to real claim IDs or option names; distinguishes manual watches from source/DOI `auto_recheckable` metadata events |
...
| `provenance_manifest.json` | seal (optional) | SHA-256 of every artifact + copied verdict, written by `verify.py --seal` (integrity, not authenticity) |
| `refresh_diff.json` / `refresh_report.md` | recheck (optional) | Deterministic before/after diff of a re-check, written by `verify.py --recheck --write` (retractions/supersession/availability; "what changed since") |
