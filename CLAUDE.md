...
# Seal the bundle (hash every artifact -> provenance_manifest.json). Optional;
# run as the last step, and composes with --write (verify --write --seal):
...
PASS means "byte-identical to the last seal," **not** "sealed by a trusted party" — an
unsigned manifest can be regenerated by anyone with write access. Note: regenerating
`storm_council_report.html` after sealing (e.g. to render the provenance section) makes
`--check-seal` report that one file as `altered`; seal as the true last step for a clean
round-trip.
